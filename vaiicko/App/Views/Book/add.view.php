...
<div class="container">
    ...
    <div id="bookAddRoot" data-category-url="<?= $link->url('category.store') ?>"
         data-genre-url="<?= $link->url('genre.store') ?>" data-redirect-url="<?= $link->url('book.manage') ?>">
        <div id="bookFormFeedback" class="ajax-error"></div>
        <form method="post" action="<?= $link->url('book.store') ?>" enctype="multipart/form-data">
            <?php if (isset($book)): ?>
                <input type="hidden" name="id"
                       value="<?= htmlspecialchars(isset($book) ? ($book->getId() ?? '') : '') ?>">
            <?php endif; ?>
            ...
            <div class="mb-3 row">
                <div class="col-md-6">
                    ...
                    <input id="isbn" name="isbn" type="text" class="form-control" required
                           value="<?= htmlspecialchars(isset($book) ? ($book->getIsbn() ?? '') : '') ?>">
                </div>
                <div class="col-md-6">
                    <label for="year_published" class="form-label">Rok vydania</label>
                    <input id="year_published" name="year_published" type="number" class="form-control" required
                           min="1000" max="<?= date('Y') ?>" step="1"
                           value="<?= htmlspecialchars(isset($book) ? ($book->getYearPublished() ?? '') : '') ?>">
                </div>
            </div>

            <div class="mb-3">
                <label for="publisher" class="form-label">Vydavateľ</label>
                <input id="publisher" name="publisher" type="text" class="form-control" required maxlength="255"
                       value="<?= htmlspecialchars(isset($book) ? ($book->getPublisher() ?? '') : '') ?>">
            </div>
            ...
            <div class="mb-3">
                ...
                <select id="author_id" name="author_id" class="form-select" required>
                    ...
                    <?php foreach ($authors as $a): ?>
                        <?php $aid = $a->getId();
                        $selected = (isset($book) && ($book->getAuthorId() ?? '') == $aid) ? 'selected' : ''; ?>
                    ...
                    <div class="input-group">
                        <select id="category_id" name="category_id" class="form-select" required>
                            ...
                            <?php foreach ($categories as $c): ?>
                                <?php $cid = $c->getId();
                                $selected = (isset($book) && ($book->getCategoryId() ?? '') == $cid) ? 'selected' : ''; ?>
                                <option value="<?= htmlspecialchars($cid) ?>" <?= $selected ?>><?= htmlspecialchars($c->getName()) ?></option>
                            <?php endforeach; ?>
                        </select>
                        <button type="button" class="btn btn-outline-success" id="showCategoryAdd"
                                data-bs-toggle="modal" data-bs-target="#categoryModal">Nová
                        </button>
                    </div>
                </div>
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        <select id="genre_id" name="genre_id" class="form-select" required>
                            ...
                            <?php foreach ($genres as $g): ?>
                                <?php $gid = $g->getId();
                                $selected = (isset($book) && ($book->getGenreId() ?? '') == $gid) ? 'selected' : ''; ?>
                                <option value="<?= htmlspecialchars($gid) ?>" <?= $selected ?>><?= htmlspecialchars($g->getName()) ?></option>
                            <?php endforeach; ?>
                        </select>
                        <button type="button" class="btn btn-outline-success" id="showGenreAdd" data-bs-toggle="modal"
                                data-bs-target="#genreModal">Nový
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea id="description" name="description" class="form-control"
                          rows="4"><?= htmlspecialchars(isset($book) ? ($book->getDescription() ?? '') : '') ?></textarea>
            </div>
            ...
            <div class="mb-3">
                <button class="btn btn-primary"
                        type="submit"><?= isset($book) ? 'Uložiť zmeny' : 'Pridať knihu' ?></button>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="new_category_name" name="name" class="form-control"
                                   placeholder="Názov kategórie" required>
                        </div>
                        <div class="mb-3">
                            <label for="new_category_description" class="form-label">Popis</label>
                            <textarea id="new_category_description" name="description" class="form-control" rows="3"
                                      placeholder="Popis kategórie"></textarea>
                        </div>
                    </div>
                ...
                <form id="genreModalForm" action="<?= $link->url('genre.store') ?>" method="post" novalidate>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="new_genre_name" name="name" class="form-control"
                                   placeholder="Názov žánru" required>
                        </div>
                        <div class="mb-3">
                            <label for="new_genre_description" class="form-label">Popis</label>
                            <textarea id="new_genre_description" name="description" class="form-control" rows="3"
                                      placeholder="Popis žánru"></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Zrušiť</button>
