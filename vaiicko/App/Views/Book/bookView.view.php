    ...
    <div class="row">
        <div class="col-md-3 text-center">
            <div class="book-cover-wrapper mb-3">
                ...
                <?php else: ?>
                    <div class="book-cover-box">
                        <svg xmlns="http://www.w3.org/2000/svg" class="book-placeholder-svg" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
            ...
            <div class="d-grid gap-2">
                ...
                <?php if ($auth?->isLogged()): ?>
                    <form method="post" action="<?= $link->url('reservation.create', ['id' => $book->getId()]) ?>">
                        <button class="btn btn-success w-100" type="submit" <?= $available <= 0 ? 'disabled' : '' ?>>Získať</button>
                    </form>
                <?php else: ?>
                    <button class="btn btn-success w-100" type="button" <?= $available <= 0 ? 'disabled' : 'data-bs-toggle="modal" data-bs-target="#loginModal"' ?>>Získať</button>
                <?php endif; ?>
            </div>
        </div>
        <div class="col-md-9">
